...
- [ ] **1.1 Remove redundant instructions**  
  ...
  > _Example:_ "Be concise. Keep your answer short. Do not write long responses." → "Be concise."
...
- [ ] **1.2 Compress few-shot examples**  
  ...
  > _Benchmark:_ Each removed example saves its full token count on every single request. A 200-token example at 10K requests/day = 2M tokens/day saved.
...
- [ ] **1.3 Remove meta-instructions the model already follows**  
  ...
  > _Test:_ Remove the instruction, run 50 samples, and check if behavior changes. If not, it's safe to delete.
...
- [ ] **1.4 Move static context to prompt templates**  
  ...
  > _Technique:_ Use your gateway's prompt-template feature or a simple string interpolation layer. Cache the rendered template to avoid re-tokenization.
...
- [ ] **2.1 Request structured output**  
  ...
  > _Example:_ Instead of "Explain the sentiment and list the entities," use:
  >
  > ```
...
- [ ] **2.2 Set explicit output length constraints**  
  ...
  > _Example:_ "Respond in ≤3 sentences." or "Return only the category label, nothing else."
  > _Also:_ Set `max_tokens` in the API call as a hard backstop. Don't rely on the instruction alone.
...
- [ ] **3.1 Test on ≥100 representative cases**  
  ...
  > _Why 100?_ At n=100, you can detect a 5-percentage-point quality drop with ~80% statistical power.
...
- [ ] **3.3 A/B test at 10% traffic before full rollout**  
  ...
  > _Rollback trigger:_ Quality score drops >2 points, or error rate increases by >50%.
