...
import {
  providerPresets, exportScenarioCSV,
  calcRAGCost, calcRoutingSavings, calcBudgetBurnRate, calcCachingROI, calcTCO,
} from "@/tokenops/data";
  ...
  return (
    <section className="stack">
      <div className="page-heading">
        ...
        <p>Six intelligent calculators: blended savings, RAG cost, model routing, budget burn rate, prompt caching ROI, and total cost of ownership.</p>
      </div>
...
function NumField({ label, value, set, step }: { label: string; value: number; set: (n: number) => void; step?: number }) {
  return (
    <label>
      {label}
      <input type="number" value={value} step={step ?? 1} onChange={(e) => set(Number(e.target.value))} />
    </label>
  );
}
...
function RAGCalculator() {
  ...
  const r = useMemo(() => calcRAGCost({
    docsPerQuery: docs, avgChunkTokens: chunk, queriesPerDay: qpd,
    systemPromptTokens: sys, avgOutputTokens: out,
    modelInputPrice: pIn, modelOutputPrice: pOut,
    cacheHitRate: hit, cachePriceDiscount: disc,
  }), [docs, chunk, qpd, sys, out, pIn, pOut, hit, disc]);
  return (
    <div className="calc-card">
      <h2>RAG Pipeline Cost Estimator</h2>
      <p>Model the true cost of retrieval-augmented generation including chunk context, system prompts, and cache hit rates.</p>
      <div className="calc-grid">
        ...
        <div className="calc-results">
          <div className="row"><span>Input tokens / query</span><strong>{fmt(r.inputTokensPerQuery)}</strong></div>
          <div className="row"><span>Cost per query</span><strong>{usd(r.costPerQuery, 4)}</strong></div>
          <div className="row"><span>Daily cost</span><strong>{usd(r.dailyCost)}</strong></div>
          <div className="row"><span>Monthly (no cache)</span><strong>{usd(r.monthlyCost)}</strong></div>
          <div className="row accent"><span>Monthly (with cache)</span><strong>{usd(r.monthlyCostWithCache)}</strong></div>
          <div className="row"><span>Cache savings</span><strong>{usd(r.cacheSavings)}</strong></div>
        </div>
      </div>
    </div>
  );
...
function RoutingCalculator() {
  ...
  const r = useMemo(() => calcRoutingSavings({
    totalCallsPerMonth: calls, premiumFraction: premF, cheapFraction: cheapF,
    avgInputTokens: inT, avgOutputTokens: outT,
    premiumInputPrice: pI, premiumOutputPrice: pO,
    cheapInputPrice: cI, cheapOutputPrice: cO,
  ...
  return (
    <div className="calc-card">
      ...
      <p>Calculate monthly and annual savings from routing a fraction of traffic to cheaper models.</p>
      <div className="calc-grid">
        <form onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="calc-results">
          <div className="row"><span>Baseline / month</span><strong>{usd(r.baselineMonthlyCost)}</strong></div>
          <div className="row"><span>Optimised / month</span><strong>{usd(r.optimisedMonthlyCost)}</strong></div>
          <div className="row accent"><span>Monthly savings</span><strong>{usd(r.monthlySavings)}</strong></div>
          <div className="row"><span>Annual savings</span><strong>{usd(r.annualSavings)}</strong></div>
          <div className="row"><span>Savings rate</span><strong>{r.savingsPercent.toFixed(1)}%</strong></div>
          <span className={`calc-status ${status}`}>{status === "ok" ? "Excellent" : status === "warn" ? "Good" : "Marginal"}</span>
        </div>
      </div>
    </div>
  );
...
function BudgetCalculator() {
  ...
  const r = useMemo(() => calcBudgetBurnRate({
    monthlyBudgetUSD: budget, currentDayOfMonth: day, spendToDate: spend, projectedGrowthRate: 0,
  }), [budget, day, spend]);
  const status = r.status === "on-track" ? "ok" : r.status === "warning" ? "warn" : "bad";
  return (
    <div className="calc-card">
      <h2>Budget Burn Rate Monitor</h2>
      <p>Project end-of-month spend from current burn, flag warnings, and compute daily budget targets.</p>
      <div className="calc-grid">
        <form onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="calc-results">
          <div className="row"><span>Daily burn</span><strong>{usd(r.dailyBurnRate)}</strong></div>
          <div className="row"><span>Recommended daily</span><strong>{usd(r.recommendedDailyBudget)}</strong></div>
          <div className="row"><span>Projected month-end</span><strong>{usd(r.projectedMonthEnd)}</strong></div>
          <div className="row accent"><span>Budget utilisation</span><strong>{r.budgetUtilization.toFixed(1)}%</strong></div>
          <div className="row"><span>Days until budget hit</span><strong>{r.daysUntilBudgetHit ?? "—"}</strong></div>
...
function CachingCalculator() {
  ...
  const r = useMemo(() => calcCachingROI({
    systemPromptTokens: sys, callsPerDay: calls,
    modelInputPrice: pIn, cacheWritePrice: wPrice, cacheReadPrice: rPrice, cacheHitRate: hit,
  }), [sys, calls, pIn, wPrice, rPrice, hit]);
  return (
    <div className="calc-card">
      <h2>Prompt Caching ROI</h2>
      <p>Quantify savings from Anthropic or OpenAI prompt caching given hit rate, write cost, and call volume.</p>
      <div className="calc-grid">
        <form onSubmit={(e) => e.preventDefault()}>
          <NumField label="System prompt tokens" value={sys} set={setSys} />
        ...
        <div className="calc-results">
          <div className="row"><span>Baseline daily</span><strong>{usd(r.baselineDailyCost, 2)}</strong></div>
          <div className="row"><span>Cached daily</span><strong>{usd(r.cachedDailyCost, 2)}</strong></div>
          <div className="row accent"><span>Daily savings</span><strong>{usd(r.dailySavings, 2)}</strong></div>
          <div className="row"><span>Monthly savings</span><strong>{usd(r.monthlySavings)}</strong></div>
          <div className="row"><span>Break-even calls</span><strong>{Number.isFinite(r.breakEvenCalls) ? fmt(r.breakEvenCalls) : "—"}</strong></div>
        </div>
      </div>
    </div>
  );
...
function TCOCalculator() {
  ...
  const r = useMemo(() => calcTCO({
    monthlyTokenCostUSD: tok, monthlyInfraUSD: infra,
    monthlyEngineeringHours: engH, engineerHourlyRate: engR,
    monthlyOperationsHours: opsH, opsHourlyRate: opsR,
    monthlyRevenueLift: rev, monthlyChurnReduction: churn,
  }), [tok, infra, engH, engR, opsH, opsR, rev, churn]);
  const status = r.roi > 100 ? "ok" : r.roi > 0 ? "warn" : "bad";
  return (
    <div className="calc-card">
      <h2>LLM Feature TCO &amp; ROI</h2>
      <p>Go beyond token cost — include infra, engineering, and ops to compute true ROI and payback period.</p>
      <div className="calc-grid">
        <form onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="calc-results">
          <div className="row"><span>Total monthly cost</span><strong>{usd(r.totalMonthlyCost)}</strong></div>
          <div className="row"><span>Total monthly benefit</span><strong>{usd(r.totalMonthlyBenefit)}</strong></div>
          <div className="row accent"><span>Net cashflow / mo</span><strong>{usd(r.netMonthlyCashflow)}</strong></div>
          <div className="row"><span>ROI</span><strong>{r.roi.toFixed(1)}%</strong></div>
          <div className="row"><span>Token cost share</span><strong>{r.tokenCostFraction.toFixed(1)}%</strong></div>
          <div className="row"><span>Payback</span><strong>{r.paybackMonths ? `${r.paybackMonths} mo` : "—"}</strong></div>
          <span className={`calc-status ${status}`}>{status === "ok" ? "Strong ROI" : status === "warn" ? "Positive" : "Underwater"}</span>
        </div>
      </div>
    </div>
  );
